...
; ===== ===== =====
; dodane miejsca w kodzie wystepuja w roznych miejscach,
; dlatego sa opatrzone komentarzem ' ; DODANE '
; ===== ===== =====
...
; ----- DODANE -----
;; lazy-lets
; wszystkie operacje i struktura sa takie same
; jak zwykle lety. jedyne co, to musimy otagowac
; inaczej lazy-lety, zeby inaczej je ewaluowac
...
; lazyboxes
; lazybox sklada sie z tagu, wyrazenia i srodowiska.
; jest to struktura w ktorej przechowujemy wyrazenie
; czekajace na obliczenie (wraz ze srodowiskiem)
; w odpowiednim momencie
(define (lazybox? t)
  (tagged-tuple? 'lazybox 3 t))

(define (lazybox-expr lb)
  (cadr lb))

(define (lazybox-env lb)
  (caddr lb))

(define (lazybox-cons expr env)
  (list 'lazybox expr env))
...
(define (expr? t)
  (or (const? t)
      ...
      (and (or (let? t) (lazy-let? t)) ; <<===== DODANE
           (expr? (let-expr t))
...
;; environments
; srodowisko to lista par:
; a) zmienna-wyrazenie (dolaczone tam przez zwykle lety)
; b) zmienna-lazybox (dolaczone tam przez lazylety)
(define empty-env
  null)
...
; jesli znajdziesz klucz i wartoscia jest lazybox,
; to oblicz go w jego srodowisku (ktore mial)
(define (find-in-env x env)
  (cond [(null? env) (error "undefined variable" x)]
        [(eq? x (caar env))
         (if (lazybox? (cadar env))         ;
             (eval-env                      ;
                (lazybox-expr (cadar env))  ; DODANE
                (lazybox-env (cadar env)))  ;
             (cadar env))]                  ;
...
(define (eval-env e env)
  (cond [(const? e)
        ...
        [(lazy-let? e)                                 ;
         (eval-env (let-expr e)                        ; DODANE
                   (env-for-lazylet (let-def e) env))] ;
        [(my-null? e)
         null]
        [(cons? e)
...
(define (env-for-lazylet def env)          ;
  (add-to-env                              ;
    (let-def-var def)                      ; DODANE
    (lazybox-cons (let-def-expr def) env)  ;
    env))                                  ;
...
#|
; TESTY
(eval '(lazy-let (x (lambda (x) (+ x x))) (x 2)))
; --> 4
...
; nieefektywne leniwe
(eval '(lazy-let [x (+ 1 2 3 4 5 6 7 8 9)]
          (* x x x x x x x x x x)))

; efektywne gorliwe
(eval '(let [x (+ 1 2 3 4 5 6 7 8 9)]
          (* x x x x x x x x x x)))
