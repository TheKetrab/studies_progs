...
(define (val->bool s)
  (cond [(eq? s 'true)  true]
        [(eq? s 'false) false]
        [else (error "could not convert symbol to bool" s)]))
...
; ===== ===== =====
; prymitywne procedury generujace
; wyrazenia w naszym nowym jezyku
(define (random-op)
  (let ((chosen-op (random 4)))
    (cond [(= chosen-op 0) ''+]
          [(= chosen-op 1) ''-]
          [(= chosen-op 2) ''*]
          [(= chosen-op 3) ''/])))
...
(define (random-expr)
  ; 30% prawd, wiec w praktyce nie bedzie sie to zapetlac
  (if (not (= (random 3) 0))
      (random 100)
      `(cons ,(random-op) (cons ,(random-expr) (cons ,(random-expr) null)))))
...
; stala to nie para i nie znak operatora
(let (ml-const? (lambda (e)
     (cond [(pair? e) false]
           [(ml-op? e) false]
           [true true])))


; binop to lista dl 3 i na poczatku jest operator
; --> (cons 'op (cons x (cons y null)))
;            VVV----- symulacja ANDa
(let (ml-binop? (lambda (e)
     (if (pair? e)
         (if (ml-op? (car e))
             (if (pair? (cdr e))
                 (if (pair? (cdr (cdr e)))
                     (if (null? (cdr (cdr (cdr e))))
                         true
                         false)
                     false)
                 false)
...
(let (ml-binop-op (lambda (e)
     (car e)))

(let (ml-binop-left (lambda (e)
     (car (cdr e))))

(let (ml-binop-right (lambda (e)
     (car (cdr (cdr e)))))

(let (ml-binop-cons (lambda (op l r)
     (cons op (cons l (cons r null)))))
                     
(let (ml-expr? (lambda (e)
     (cond [(ml-const? e) true]
           [(ml-binop? e) true]
           [true false])))

(let (ml-eval (lambda-rec (ml-eval e)
     (cond [(ml-const? e) e]
           [(ml-binop? e)
            (cond [(eq? (ml-binop-op e) '+)
                   (+ (ml-eval (ml-binop-left e))
                      (ml-eval (ml-binop-right e)))]
                  [(eq? (ml-binop-op e) '-)
                   (- (ml-eval (ml-binop-left e))
                      (ml-eval (ml-binop-right e)))]
                  [(eq? (ml-binop-op e) '*)
                  ...
                  [(eq? (ml-binop-op e) '/)
                   (/ (ml-eval (ml-binop-left e))
                      (ml-eval (ml-binop-right e)))])])))
; -- program
  ; ---> czasem moze wyskoczyc blad, bo losujaca procedua moze
  ;      wygenerowac wyrazenie, w ktorym mamy dzielenie przez zero
  ;      (chociaz mimo wielu testow tak mi sie jeszcze nie zdarzylo)
  (if (ml-expr? ,test)
      (ml-eval ,test)
      false)
